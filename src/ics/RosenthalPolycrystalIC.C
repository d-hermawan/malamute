...
RosenthalPolycrystalIC::validParams()
{
  ...
  MooseEnum structure_options("grains voids temperature");
...
RosenthalPolycrystalIC::value(const Point & p)
{
    ...
    Real grain_value = 0;
    if (_current_node)
      grain_value = _poly_ic_uo.getNodalVariableValue(_op_index, *_current_node);
    else
      grain_value = _poly_ic_uo.getVariableValue(_op_index, p);
    Real void_value = 0;
    Real temperature = _rosenthal_uo.value(p);
    Real _Tm = _rosenthal_uo.melting_temp();
    void_value = _invalue * (0.5*std::tanh(0.25*(temperature-_Tm))+0.5) + (1-(0.5*std::tanh(0.25*(temperature-_Tm))+0.5))*_outvalue;
    ...
    case 1: // assigning values for voids (concentration)
      value = void_value;
      break;
    case 2: // assigning values for temperature
      value = temperature;
    }
    return value;
}